...
void readSensor(){
  Sensor.check();
  dat[1]=Sensor.getFIFORed(); //swap to IR
  dat[0]=Sensor.getFIFOIR(); //swap to Red
...
void setLeds(uint8_t Red, uint8_t IR){
   Sensor.setPulseAmplitudeRed(IR); //swap to IR
   Sensor.setPulseAmplitudeIR(Red); //swap to Red
}//setLeds
